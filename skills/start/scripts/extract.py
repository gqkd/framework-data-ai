...
def main() -> int:
    ...
    # The documents that contributed nothing go at the top, before anything that did.
    # `inventory.json` has always recorded them, but nothing in the skill tells you to open
    # it, and a gap you have to know to go looking for reads as completeness: a scanned PDF
    # waiting for OCR and a file the filesystem refused both leave a corpus that looks
    # whole. Whoever classifies these claims has to know which ones never arrived.
    if silent:
        md += [f"## {len(silent)} of {len(files)} documents produced no text", "",
