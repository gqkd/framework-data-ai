...
The ids above are the chain as the log writes it, and a log writes them bare: it sits under
`products/<p>/`, so the directory has already said whose they are. The third line is the one
that leaves the product -- a `DEC` lives at the root -- and there the reference carries the
qualifier: `derives_from: [<product>:ANA-014]`. Bare is `FM002` from 3.0.0.
...
4. **One `OPEN.md` per product, and this is the register that does *not* follow rule 2.**
   ...
   register, ordered by cost across every register at once. `REG006` reports a product with
   no register of its own, `REG007` a numbering that restarted, `REG008` an entry filed under
   the wrong product, `REG011` an entry at the root that does not say which products it
   binds and `REG013` one that binds a single product and belongs beside it, and `REG009` a
   date in an entry's `trigger` -- what closes the window
