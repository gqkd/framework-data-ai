...
Only two can block a repository on its first day: `FM001`, front matter that parses, and
`FM002`, front matter that means something. Everything a repository holds is read through
those two, so a document failing either is not being checked rather than checked and found
wanting. Others are `error` as well, and none of them can fire until a project has opted in
