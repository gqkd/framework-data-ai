...
| `resolve/ordering-a` | Nine open decisions. Two are filed under a heading that contradicts the cost they declare, one depends on an entry that exists nowhere, one is missing `Default in force` entirely. `OD003` reports four of them; it used to report three, and the one it missed was the entry with no `Default in force` at all — the prose check looked for the word `none` and an absent field does not contain it. |
