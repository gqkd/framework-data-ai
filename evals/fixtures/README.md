...
| | |
|---|---|
| `audit/dirty-repo` | Real defects: front matter that does not parse, a `derives_from` pointing at a decision nobody wrote, a decision superseded by one that never moved it to `superseded`, a product with no open register of its own, an open entry carrying a date where its trigger belongs, one at the root that does not say which products it binds, a decision that does not say what it leaves open, and a data contract citing a glossary term nobody defined. Four errors and eighteen warnings, by construction. |
