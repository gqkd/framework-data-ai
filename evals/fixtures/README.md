...
| | |
|---|---|
| `audit/dirty-repo` | Real defects: front matter that does not parse, a `derives_from` pointing at a decision nobody wrote, a decision superseded by one that never moved it to `superseded`, and a product with no open register of its own. Four errors and thirteen warnings, by construction. |
